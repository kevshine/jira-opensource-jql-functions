 ...
 * 
 */
public class ParentWithIssueTypeOf extends AbstractJqlFunction {

	
    private final UserIssueHistoryManager userIssueHistoryManager;

    public ParentWithIssueTypeOf(UserIssueHistoryManager  userIssueHistoryManager)
    {
        this.userIssueHistoryManager = userIssueHistoryManager;
    }
    
	private static final Logger log = LoggerFactory.getLogger(ParentWithIssueTypeOf.class);
	...
	public List<QueryLiteral> getValues(@NotNull QueryCreationContext queryCreationContext, @NotNull FunctionOperand operand, @NotNull TerminalClause terminalClause) {
		...
		try {
			for (Iterator<String> iterator = arguments.iterator(); iterator.hasNext();) {
				...
				if (string.contains(",")) {
					String[] issueTypes = string.split(",");
					builder.where().issueType(issueTypes);
				} else {
					builder.where().issueType(string);
				}
